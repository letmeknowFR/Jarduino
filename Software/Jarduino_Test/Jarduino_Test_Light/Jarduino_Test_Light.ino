...
#define PIN_RELAY1 9                               // Pin du relais de la lampe
...
void loop() {
  ...
  if (cmd.equals("off")) {                         // On compare l'instruction avec "off"
    digitalWrite(PIN_RELAY1, LOW);                 // Met un niveau logique BAS sur le pin spécifié
    lcd.clear();                                   // Nettoyage de l'écran
    lcd.print("Lampe OFF");                        // Affichage sur l'écran LCD
    Serial.println("Lampe OFF");                   // Affichage sur le moniteur série
  }
}
/***************************** FIN LOOP **************************/
